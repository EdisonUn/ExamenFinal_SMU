package com.example.examenfinal_smo;
...
import com.example.examenfinal_smo.ml.Modelfacultadesv2;
...
public class MainActivity extends AppCompatActivity implements OnSuccessListener<Text>, OnFailureListener, ImageReader.OnImageAvailableListener{
    ...
    public void PersonalizedModel(View v) {
        try {
            Modelfacultadesv2 model = Modelfacultadesv2.newInstance(getApplicationContext());
            TensorImage image = TensorImage.fromBitmap(mSelectedImage);

            Modelfacultadesv2.Outputs outputs = model.process(image);
    ...
    private void processImage() {
        ...
        try {
            ...
            String res="";
            for (int i = 0; i < probability.size(); i++)
                //Aqui para comprobar una linea
                /*Este if borrar*/if (probability.get(i).getScore()>=0.5)
            {
                res = res + probability.get(i).getLabel() +  " " +  probability.get(i).getScore()*100 + " % \n";
            }
